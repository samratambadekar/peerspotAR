	...
	/* Start: Global variables created for accessibility */
	...
	function getRandomNumber(targetId)
	{
		document.getElementById(targetId).text = Math.floor(Math.random()*18);
	}
	...
	function showTrolleys(j)
	{
		if(j == 1)
		{
			...
			function locationChangedTrolley(lat, lon, alt, acc)
			{
				...
				//trolleyImg = new AR.ImageResource("./img/trolley.png");
				//var trolleyImgDrawable = new AR.ImageDrawable(trolleyImg, 2.5, {enabled: true});
				...
				for(i=0; i < buses.length; i++)
				{
					...
					if(buses[i].color == "yellow")
					{
						...
						trolleyGeoObjects[i] = new AR.GeoObject(busLocation, {drawables: {cam: trolleyImgDrawable[i]}} );
		...
		if(j == 0)
		{
			trolleyFlag = false;
			//trolleyImg.destroy();
			//trolleyImgDrawable.destroy();
			//trolleysGeoObjects.destroy();
			//destroyAllObjects();
			for(var i=0;i<trolleysGeoObjects.length;i++)
			{
				trolleyImgDrawable[i].destroy();
				trolleysGeoObjects[i].destroy();
			}
	...
	function showRedBus(j)
	{
		if(j == 1)
		{
			...
			function locationChangedTrolley(lat, lon, alt, acc)
			{
				...
				//redBusImg = new AR.ImageResource("./img/redRoute.png");
				//var redBusImgDrawable = new AR.ImageDrawable(redBusImg, 2.5, {enabled: true});
				...
				for(i=0; i < buses.length; i++)
				{
					...
					if(buses[i].color == "red")
					{
						...
						var busLocation = new AR.GeoLocation(buses[i].lat, buses[i].lng);
						redBusGeoObjects[i] = new AR.GeoObject(busLocation, {drawables: {cam: redBusImgDrawable[i]}} );
						//alert(busLocation.distanceToUser() + " meters");
					}
				}
		...
		if(j == 0)
		{
			redBusFlag = false;
			//redBusImg.destroy();
			//redBusImgDrawable.destroy();
			//redBusGeoObjects.destroy();
			//destroyAllObjects();
	...
	function showBlueBus(j)
	{
		if(j == 1)
		{
			...
			function locationChangedTrolley(lat, lon, alt, acc)
			{
				...
				//blueBusImg = new AR.ImageResource("./img/blueRoute.png");
				//var blueBusImgDrawable = new AR.ImageDrawable(blueBusImg, 2.5, {enabled: true});
				...
				for(i=0; i < buses.length; i++)
				{
					...
					if(buses[i].color == "blue")
					{
						...
						var busLocation = new AR.GeoLocation(buses[i].lat, buses[i].lng);
						blueBusGeoObjects[i] = new AR.GeoObject(busLocation, {drawables: {cam: blueBusImgDrawable[i]}} );
						//alert(busLocation.distanceToUser() + " meters");
					}
				}
				//var closestStopLocation = closestStop("blue");
				//alert("Closest Blue Route Stop is " + closestStopLocation[1] + " at " + parseFloat(closestStopLocation[0]) + " meters");
		...
		if(j == 0)
		{
			blueBusFlag = false;
			//blueBusImg.destroy();
			//blueBusImgDrawable.destroy();
			//blueBusGeoObjects.destroy();
			//destroyAllObjects();
	...
	function showGreenBus(j)
	{
		if(j == 1)
		{
			...
			function locationChangedTrolley(lat, lon, alt, acc)
			{
				...
				//greenBusImg = new AR.ImageResource("./img/greenRoute.png");
				//var greenBusImgDrawable = new AR.ImageDrawable(greenBusImg, 1.0, {enabled: true});
				...
				for(i=0; i < buses.length; i++)
				{
					...
					if(buses[i].color == "green")
					{
						...
						var busLocation = new AR.GeoLocation(buses[i].lat, buses[i].lng);
						var greenBusGeoObjects[i] = new AR.GeoObject(busLocation, {drawables: {cam: greenBusImgDrawable[i]}} );
						//alert(busLocation.distanceToUser() + " meters");
					}
				}
		...
		if(j == 0)
		{
			greenBusFlag = false;
			//greenBusImg.destroy();
			//greenBusImgDrawable.destroy();
			//greenBusGeoObjects.destroy();
			//destroyAllObjects();
	...
	function showBike(j)
	{
		...
		//bikeImg = new AR.ImageResource("./img/bike.png");
		//var bikeImgDrawable = new AR.ImageDrawable(bikeImg, 2, {enabled: true});
...
/*
					<div><label>Name: " + bikeStopLocation[i][0] +
					"</label><br><label>Distance: " + parseFloat(bikeStopLocation[i][2]) + " meters</label><br><label>Numbaer of bikes: " +
					bikeStopLocation[i][1] + "</label></div>"}, 30, {
					  offsetX : 1,
					  horizontalAnchor : AR.CONST.HORIZONTAL_ANCHOR.LEFT,
					  opacity : 0.9
					});
...
*/
					//bikeImgDrawable.push(new AR.Label("Name: " + bikeStopLocation[i][0], 1, {enabled: true, style : {textColor : '#FFC100', backgroundColor : '#FFFFFF80'}}));
					...
					//bikeImgDrawable.html += "<div>Another div</div>";
/*
				var bikeStopLocation = closestBikeStop();
...
//name count dist isnot
		...
		if(j == 0)
		{
			...
			bikeImgDrawable = new AR.HtmlDrawable({html:""}, 18, {
								  offsetX : 1,
								  horizontalAnchor : AR.CONST.HORIZONTAL_ANCHOR.LEFT,
								  opacity : 0.9
					});
				*/
			//bikeImg.destroy();
			//bikeImgDrawable.destroy();
			//bikesGeoObjects.destroy();
			//bikesGeoObjects.remove();
		}
...
/*
	...
	*/
	...
	function placeMarker(location, map)
	{
		...
		//document.getElementById('search_field').value = locationName;
        ...
        geocoder.geocode({'latLng': latlng}, function(results, status) {
          if (status == google.maps.GeocoderStatus.OK) {
            if (results[1]) {
              ...
              locName = results[1].formatted_address;
	...
	function showClosestBusStopsMarker(closestBusStopLocation)
	{
		...
		//alert(closestBusStopLocation[1].length);
		// Called if the location changes
		function locationCustomLocation(lat, lon, alt, acc)
		{
			...
			for(var i=0; i < closestBusStopLocation.length; i++)
			{
				...
				//alert(parseFloat(closestBusStopLocation[i][2].latitude));
